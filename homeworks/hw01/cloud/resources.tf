...
resource "yandex_compute_instance" "linux-vm" {
  ...
  provisioner "local-exec" {
    command     = "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i '${self.network_interface.0.nat_ip_address},' -u '${local.user}'  --private-key '${local.rsa_key}' ./ansible/'${local.playbook}'.yml"
